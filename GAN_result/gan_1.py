...
if __name__ == "__main__":
    ...
    os.mkdir('./gan_1/{}_real'.format(now.strftime("%Y%m%d-%H%M%S")))
    plt.figure()
    plt.plot(epochs, acc, 'bo', label='Training acc')
    plt.plot(epochs, val_acc, 'b', label='Validation acc')
    plt.title('Training and validation accuracy')
    plt.legend()  # 作用是加上图例，很有必要
    plt.savefig("./gan_1/{}_real/real_TRandVAL_acc.png".format(now.strftime("%Y%m%d-%H%M%S")))
    plt.figure()  # 创建新图
    plt.plot(epochs, loss, 'bo', label='Training loss')
    plt.plot(epochs, val_loss, 'b', label='Validation loss')
    plt.title('Training and validation loss')
    plt.legend()
    plt.savefig("./gan_1/{}_real/real_TRandVAL_loss.png".format(now.strftime("%Y%m%d-%H%M%S")))
    ...
    for threshold in threshold_list:
        ...
        os.mkdir('./gan_1/{}_gen_threshold({})'.format(now.strftime("%Y%m%d-%H%M%S"), threshold))
        plt.figure()
        plt.plot(epochs, acc, 'bo', label='Training acc')
        plt.plot(epochs, val_acc, 'b', label='Validation acc')
        plt.title('Training and validation accuracy')
        plt.legend()  # 作用是加上图例，很有必要
        plt.savefig("./gan_1/{}_gen_threshold({})/gen+real_TRandVAL_acc.png".format(now.strftime("%Y%m%d-%H%M%S"),
                                                                                    threshold))
        plt.figure()  # 创建新图
        plt.plot(epochs, loss, 'bo', label='Training loss')
        plt.plot(epochs, val_loss, 'b', label='Validation loss')
        plt.title('Training and validation loss')
        plt.legend()
        plt.savefig("./gan_1/{}_gen_threshold({})/gen+real_TRandVAL_loss.png".format(now.strftime("%Y%m%d-%H%M%S"),
                                                                                     threshold))
        ...
        file_dir = "./gan_1/{}_gen_threshold({})/test.txt".format(now.strftime("%Y%m%d-%H%M%S"), threshold)
    ...
    plt.legend()
    plt.savefig("./gan_1/mean_acc and max_acc.png")
    ...
    plt.legend()
    plt.savefig("./gan_1/acc_STD and loss_STD.png")

    plt.figure()
    plt.plot(threshold_list, mean_loss_list, 'r', label="mean loss")
    plt.title('mean_loss')
    plt.legend()
    plt.savefig("./gan_1/mean_loss.png")
